...
bool Shell::getCommand() {
	...
	if (nrOfCommands > 0) {
		...
		case 2: // ls
            ...
            if (nrOfCommands < 2)
                std::cout << fileSystem.ls(absolutePath(currentDir));
            else
                std::cout << fileSystem.ls(absolutePath(commandArr[1]));

			break;
		case 3: // create	
			std::cout << "Enter file contents: \n";
			std::getline(std::cin, fileContent);
			std::cout << fileSystem.create(absolutePath(commandArr[1]), fileContent);
			break;
		case 4: // cat
			std::cout << fileSystem.cat(absolutePath(commandArr[1]));
			break;
		case 5: // save
		...
		case 6: // load
			...
			break;
		case 7: // rm
			std::cout << fileSystem.rm(absolutePath(commandArr[1]));
			break;

		case 8: // copy
			std::cout << fileSystem.copy(absolutePath(commandArr[1]), absolutePath(commandArr[2]));
			break;

		case 9: // append
			std::cout << fileSystem.append(absolutePath(commandArr[1]), absolutePath(commandArr[2]));
			break;

		case 10: // rename
            std::cout << fileSystem.rename(absolutePath(commandArr[1]), absolutePath(commandArr[2]));
			break;

		case 11: // mkdir
			std::cout << fileSystem.mkdir(absolutePath(commandArr[1]));
			break;
		...
		case 15: // chmod
			if (commandArr[2].length() != 0){
				...
				std::cout << fileSystem.chmod(absolutePath(commandArr[1]), perm);
			} else {
